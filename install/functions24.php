<?php
/*
    Functions - GuppY PHP Script - version 2.4
...
if (stristr($_SERVER["SCRIPT_NAME"], "functions24.php")) {
...
function ReadDocCounter($id) {
  $DataDB = ReadCounter(DBBASE.$id.DBEXT);
  return $DataDB;
}

function WriteDocCounter($id,$DataDB) {
  WriteCounter(DBBASE.$id.DBEXT,$DataDB);
}
...
function UpdateDBnews() {
  global $lang,$lng,$site,$serviz,$web6,$web7,
  ...
  if ($serviz[16] == "on") {
    ...
    for ($i = 0; $i < count($dbwork1); $i++) {
      ReadDoc($dbwork1[$i][1]);
      $dbwork2[$i][0] = $fieldb1." - ".$par1." ".$author." ".$le1." ".FormatDate($creadate);
      $dbwork2[$i][1] = $fieldc1;
      $dbwork2[$i][2] = $sito."news.php?lng=".$lng."&id=".$fileid;
      if ($lang[1] != "") {
        $dbwork2[$i][3] = $fieldb2." - ".$par2." ".$author." ".$le2." ".FormatDate($creadate);
        $dbwork2[$i][4] = $fieldc2;
...
function ActionOnFields($action,$data) {
  ...
  if ($data[0] == TYP_ART || $data[0] == TYP_LINKS || $data[0] == TYP_DNLOAD || $data[0] == TYP_PHOTO || $data[0] == TYP_FAQ) {
    UpdateDBdtb($data[0]);
  }
...
function eMailTo($eSubject,$eMessage,$eTo) {
  ...
  if ($supervision[5] == "standard") {
    @mail($eTo,$eSubject,$eMessage,"From: ".$eFrom,"-f".$eFrom);
  }
  else {
